...
$(this).fadeIn(500)})};$.fn.mauGallery.defaults={columns:3,lightBox:!0,lightboxId:null,showTags:!0,tagsPosition:"bottom",navigation:!0,};$.fn.mauGallery.listeners=function(options){$(".gallery-item").on("click keydown",function(event){if(event.type==="keydown"&&event.keyCode!==13){return}
if(options.lightBox&&$(this).prop("tagName")==="IMG"){$.fn.mauGallery.methods.openLightBox($(this),options.lightboxId)}else{return}});$(".gallery").on("click",".nav-link",$.fn.mauGallery.methods.filterByTag);$(".gallery").on("keydown",(e)=>{if(!e.target.querySelector(".nav-link"))return;console.log(e.target.querySelector("span"));if(e.keyCode===13){console.log("allez");e.target.querySelector("span").click()}})
$(".gallery").on("click",".mg-prev",()=>$.fn.mauGallery.methods.prevImage(options.lightboxId));$(".gallery").on("click",".mg-prev",()=>$.fn.mauGallery.methods.prevImage(options.lightboxId));$(".gallery").on("click",".mg-next",()=>$.fn.mauGallery.methods.nextImage(options.lightboxId));document.addEventListener("keydown",(e)=>{if(e.keyCode===37){$.fn.mauGallery.methods.prevImage(options.lightboxId)}});document.addEventListener("keydown",(e)=>{if(e.keyCode===39){$.fn.mauGallery.methods.nextImage(options.lightboxId)}})};$.fn.mauGallery.methods={createRowWrapper(element){if(!element.children().first().hasClass("row")){element.append('<div class="gallery-items-row row"></div>')}},wrapItemInColumn(element,columns){if(columns.constructor===Number){element.wrap(`<div class='item-column mb-4 col-${Math.ceil(12 / columns)}'></div>`)}else if(columns.constructor===Object){var columnClasses="";if(columns.xs){columnClasses+=` col-${Math.ceil(12 / columns.xs)}`}
